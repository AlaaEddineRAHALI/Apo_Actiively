...
module.exports = {
  ...
  async register(req, res) {
    const { email, name, password, description, contact_email, phone_number } =
      req.body;
    ...
    try {
      ...
      res
        .status(200)
        .json({
          message: `Organism with email ${organismHashed.email} successfully created !`,
        });
    } catch (err) {
  ...
  async login(req, res) {
    ...
    try {
      const organism = await organismDatamapper.findOrganismInfosForLogin(
        email
      );
  ...
  async profile(req, res) {
    const organism = await organismDatamapper.findOneOrganism(
      req.decodedToken.id
    );

    res.json({
      message: "PROFILE",
      connected: req.authenticated,
      user: organism,
    });
  },
  /**
  ...
  async updateProfile(req, res) {
    ...
    try {
      await organismDatamapper.updateProfile(
        organismProfil,
        req.decodedToken.id
      );
