...
module.exports = {
  ...
  async getOneOrganismActivity(req, res) {
    const activity = await activityDatamapper.findActivityByOrganism(
      req.params.id,
      req.decodedToken.id
  ...
  async getOrganismActivities(req, res) {
    const activities = await activityDatamapper.findActivitiesByOrganism(
      req.decodedToken.id
    );
    try {
      res.json(activities);
  ...
  async postOneActivity(req, res) {
    try {
      ...
      if (activityExist) {
        ...
        await activityDatamapper.createActivity(
          activityWihoutDay,
          req.decodedToken.id
        );
  ...
  async deleteOneActivity(req, res) {
    ...
    try {
      if (activityToDelete) {
        await activityDatamapper.deleteActivityByPk(
          currActivity,
          req.decodedToken.id
        );
        ...
        throw new Error(
          `Activity with id (${currActivity}) is not exists or already deleted`
        );
      }
    } catch (err) {
      res.json({ error: err.message });
  ...
  async updateOneActivity(req, res) {
    ...
    try {
      await activityDatamapper.updateActivity(
        activityToUpdate,
        req.decodedToken.id,
        activityId
      );
